...
COMPARISON_RUNTIME_ROOT = (
    _PACKAGED_MODEL_RUNTIME
    if (_PACKAGED_MODEL_RUNTIME / "models").exists()
    else PROJECT_ROOT
)
...
MODEL_IMPORT_ROOTS = []
for _root in (XJU_ROOT, COMPARISON_RUNTIME_ROOT, PROJECT_ROOT, PROJECT_ROOT.parent):
    _root = Path(_root)
...
DEFAULT_CHECKPOINT = (
    ...
    else _PACKAGED_DEFAULT_CHECKPOINT
...
def model_catalog() -> list[dict]:
    """Return the selectable thesis models and their packaged availability.

    Paths are intentionally omitted from the browser payload.  Availability is
    calculated independently for the legacy and 16-sensor schemas so the UI
    cannot present an algorithm whose matching weights were not packaged.
    """
    catalog = []
    for model_type, definition in MODEL_REGISTRY.items():
        availability = {}
        validation_loss = {}
        for schema_id in ("legacy_original", "new_collection_v11_3"):
            checkpoint = _registered_checkpoint(model_type, schema_id)
            availability[schema_id] = bool(
                checkpoint is not None and Path(checkpoint).is_file()
            )
            validation_loss[schema_id] = (
                _training_validation_loss(Path(checkpoint))
        ...
        catalog.append(
            {
                ...
                "default_epochs": 100,
                "default_patience": 10,
                "available_by_schema": availability,
                "validation_loss_by_schema": validation_loss,
            }
        )
    return catalog
...
def _training_validation_loss(checkpoint: Path) -> float | None:
    """Read the minimum validation loss recorded beside a packaged model."""
    history = checkpoint.parent / "training_history.csv"
    ...
    try:
        ...
        with history.open("r", encoding="utf-8-sig", newline="") as stream:
            for row in csv.DictReader(stream):
                value = row.get("validation_loss") or row.get("val_loss")
...
def _registered_checkpoint(model_type: str, schema_mode: str = "") -> Path | None:
    ...
    if scope == "new":
        # The authoritative 16-sensor checkpoints live in models/new.  Older
        # demo checkpoints used 19 sensor inputs and must never shadow them.
        candidates = [
...
def inspect_prediction_model(
    ...
    result = {
        **metadata,
        ...
        "validation_loss": _training_validation_loss(path),
    }
...
class OnlineIModernTCN:
    ...
    def _load(self) -> None:
        ...
        if self._profile.get("model_type") != "i_T_G":
            ...
            project_root = str(COMPARISON_RUNTIME_ROOT)
