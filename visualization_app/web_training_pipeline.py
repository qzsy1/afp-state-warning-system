...
def train_models(
    ...
    packaged_runtime = Path(__file__).resolve().parent / "model_runtime"
    if getattr(sys, "frozen", False):
        xju_root = Path(getattr(sys, "_MEIPASS")) / "model_runtime"
    else:
        xju_root = (
            packaged_runtime
            if (packaged_runtime / "shijie").exists()
    ...
    comparison_root = (
        xju_root
        if (xju_root / "models").exists()
        else source_project_root
    )
    import_roots = [xju_root, comparison_root, source_project_root]
    ...
    if selected_model_type != "i_T_G":
        ...
        comparison_root_text = str(comparison_root)
        if comparison_root_text in sys.path:
            sys.path.remove(comparison_root_text)
        sys.path.insert(0, comparison_root_text)
    try:
