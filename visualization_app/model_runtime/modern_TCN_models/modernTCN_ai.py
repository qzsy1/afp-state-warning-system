...
# 这次绝对能找到了，因为我们把原来的 XJUsorceopen 找回来了！
from layers.RevIN import RevIN
from ModernTCN_Layer import series_decomp, Flatten_Head

# 🚨 专门用于 1D 时序特征的安全 LayerNorm
...
# ================= 注意 =================
# get_bn 和 conv_bn 保留原本的 BatchNorm1d
# 因为 ReparamLargeKernelConv 的重参数化 (fuse_bn) 极其依赖 BN 的 running_mean
# 这是局部卷积块，不会造成全局的尺度崩塌
...
class ReparamLargeKernelConv(nn.Module):
    ...
    def PaddingTwoEdge1d(self, x, pad_length_left, pad_length_right, pad_values=0):
        ...
        if pad_values == 0:
            pad_left = torch.zeros(D_out, D_in, pad_length_left).to(x.device)
            pad_right = torch.zeros(D_out, D_in, pad_length_right).to(x.device)
        else:
            pad_left = torch.ones(D_out, D_in, pad_length_left).to(x.device) * pad_values
            pad_right = torch.ones(D_out, D_in, pad_length_right).to(x.device) * pad_values
...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.1):
        ...
        # 🚨 核心修复：替换为不会摧毁时间维度的 LayerNorm
        self.norm = TransposeLayerNorm(dmodel)
