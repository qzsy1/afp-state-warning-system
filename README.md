...
该仓库包含自动铺丝（AFP）系统的实时采集、I-ModernTCN 多变量预测、在线健康指标计算，以及“窗口级 → 铺层级 → 实际铺层数试样级”的状态预警界面源码。
...
- 以窗口、铺层和试样三级展示预警证据，试样级按实际已经采集的铺层数动态聚合；
- 采集数据按“保存位置/试样名+工艺参数/试样名+工艺参数组合+铺层数”保存，同时保留完整试样与单层文件；
- 对接 JSON Lines 传感器流，并显示连接和数据接收状态；
- 本地文件保存成功后，可按层事务同步至指定 MySQL 数据库，并通过关系视图查看工况—试样—独立重复—铺层关系。
...
与v1.11.0实现对应的论文基础章节位于：

`thesis_draft/AFP实时预测与三级状态预警系统核心流程说明_软件数据库集成更新版_v5.docx`

其中说明了16个实际采集通道、目标时刻绑定预测、窗口—铺层—实际铺层数试样三级CAP、任意铺层/独立重复、本地文件优先保存、MySQL外键/索引/关系视图和软件验证边界。
